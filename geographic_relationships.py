#!/usr/bin/env python3
"""
Geographic Relationships Builder
...
def create_geographic_relationships(variables):
    ...
    if project_name_exists:
        # Project is located at various geographic features
        location_fields = ['country', 'province', 'state', 'region', 'district', 'location', 'location_coordinates']
        
        for var in general_info_vars:
            col_name = var['column_name']
            col_lower = col_name.lower()
            
            if any(field in col_lower for field in location_fields):
...
def main():
    ...
    print("\nCreating geographic relationships...")
    relationships = create_geographic_relationships(variables)
    ...
    # Append to existing mining relationships
    ...
    output_path = '/dropbox/03. Knowledge Graph/mining_relationships_updated.csv'
    ...
    print(f"Saved updated mining relationships to: {output_path}")
