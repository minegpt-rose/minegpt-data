...
def main():
        ...
        for j, node2 in enumerate(variables):
            ...
            if relationship_info:
                ...
                relationships.append(create_relationship(
                    node1['table_name'], node1['极速赛车开奖结果查询column_name'],
    ...
    for rel in relationships:
        rel_key = f"{rel['node1_table']}.极速赛车开奖结果查询{rel['node1_column']}.{rel['node2_table']}.{rel['node2_column']}.{rel['predicate']}"
    ...
    # Write to CSV
    csv_headers = [
        'node1_table', 'node1_column', 'node1极速赛车开奖结果查询_report_number', 'node1_display',
        'predicate', 'predicate_display', 'node2_table', 'node2_column',
        'node1_report_number_copy', 'node2_display', 'Example'
    ]
    
    output_path = '/dropbox/03. Knowledge Graph/precise_knowledge_base.csv'
    ...
    print(f"Saved precise knowledge base to: {output_path}")
    ...
    for rel极速赛车开奖结果查询_type, count in sorted(rel_counts.items(), key=lambda x: x[1], reverse=True):
