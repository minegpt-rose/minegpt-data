#!/usr/bin/env python3
"""
Systematic Knowledge Base Builder

Go through every variable as node1, then every other variable as node2
Determine relationships systematically
"""
...
def should_create_relationship(node1, node2):
    ...
    if table1 == 'cost_table' and table2 == 'cost_table':
        if 'total' in col2_lower and any(keyword in col1_lower for keyword in ['mining', 'process', 'plant', 'tailings', 'infrastructure', 'closure']):
    ...
    if col1 == 'project_name' and table1 == 'general_information':
        if any(keyword in notes2 for keyword in ['commodity', 'mineral', 'deposit', 'geological']):
            if 'commodity' in col2_lower:
                return ('produces', 'Produces')
            elif 'deposit' in col2_lower:
                return ('has_deposit_type', 'Has Deposit Type')
            elif 'geological' in col2_lower:
...
def main():
    ...
    print("\nBuilding systematic knowledge base...")
    relationships = []
    ...
    output_path = '/dropbox/03. Knowledge Graph/systematic_knowledge_base.csv'
    ...
    print(f"Saved systematic knowledge base to: {output_path}")
